...
var arrowTypes = [DIAGONAL, MARGIN, STRAIGHT];
var ARROW = "arrow";
...
var STRAIGHT_ARROW = "arrow2";
var ARROW_BOX_CLASS = "arrow-box";
...
var ArrowsManager = class {
  ...
  drawArrow(startEl, endEl, startArrowData, endArrowData, startOffscreen, endOffscreen) {
    ...
    } else if (startArrowData.type == STRAIGHT) {
  ...
  drawDiagonalArrow(startEl, endEl, startArrowData, endArrowData) {
    ...
    if (startArrowData.isBox) {
      ...
      if (boxEl) {
        ...
        startAnchor = LeaderLine2.PointAnchor(boxEl, { x: boxWidth / 2, y: boxHeight });
      }
    }
    if (endArrowData.isBox) {
      ...
      if (boxEl) {
        const boxWidth = boxEl.offsetWidth;
        endAnchor = LeaderLine2.PointAnchor(boxEl, { x: boxWidth / 2, y: 0 });
      }
    }
  ...
  drawStraightArrow(startEl, endEl, startArrowData, endArrowData) {
    if (startEl == endEl)
      return;
    const color = colorToEffectiveColor(startArrowData.color, getArrowConfigFromView(this.view));
    const plugs = getStartEndArrowPlugs(STRAIGHT_ARROW, startArrowData.arrowArrowhead, endArrowData.arrowArrowhead);
    let startAnchor = startEl;
    let endAnchor = endEl;
    if (startArrowData.isBox) {
      const boxEl = this.findBoxElement(startEl);
  ...
  findBoxElement(el) {
    ...
    while (current && !current.hasClass("cm-line")) {
      if (current.hasClass(ARROW_BOX_CLASS)) {
        return current;
      }
    ...
    if (!line)
      return null;
    const boxes = line.querySelectorAll(`.${ARROW_BOX_CLASS}`);
    if (boxes.length === 0)
      return null;
    if (boxes.length === 1)
      return boxes[0];
  ...
  getMarginArrowStartEndAnchors(start, end, startArrowData, endArrowData, startOffscreen, endOffscreen) {
  ...
  collectArrowIdentifierPosData(arrowIdentifierPosData) {
    ...
    for (const arrowIdentifierPos of arrowIdentifierPosData) {
      ...
      if (!(identifier in result)) {
        result[identifier] = { identifier, ends: [], boxClosings: [] };
      }
      if (isBoxClosing) {
    ...
    for (const arrowIdentifierCollection of arrowIdentifierCollections) {
      ...
      if (isResolved) {
        ...
        if (arrowIdentifierCollection.boxClosings.length > 0) {
          ...
          if (start.arrowData.isBox && closingIndex < arrowIdentifierCollection.boxClosings.length) {
            ...
            const boxDeco = import_view2.Decoration.mark({
              tagName: "span",
              class: ARROW_BOX_CLASS,
              attributes: {
          ...
          for (const end of arrowIdentifierCollection.ends) {
            if (end.arrowData.isBox && closingIndex < arrowIdentifierCollection.boxClosings.length) {
              ...
              const effectiveBoxColor = endBoxColor ? colorToEffectiveColor(endBoxColor, getArrowConfigFromView(view)) : boxColor ? colorToEffectiveColor(boxColor, getArrowConfigFromView(view)) : color ? colorToEffectiveColor(color, getArrowConfigFromView(view)) : "var(--text-normal)";
              const boxDeco = import_view2.Decoration.mark({
                tagName: "span",
                class: ARROW_BOX_CLASS,
                attributes: {
      ...
      for (const [index, arrowIdentifier] of allArrowIdentifiers.entries()) {
        ...
        if (arrowIdentifier.arrowData.isBoxClosing) {
          if (shouldUnravel) {
            const deco2 = import_view2.Decoration.mark({
              tagName: "span",
            ...
            const deco2 = import_view2.Decoration.replace({
...
var EmptyWidget = class extends import_view2.WidgetType {
  constructor() {
    super();
  }
  eq(other) {
...
var InvisibleCircle = class extends import_view2.WidgetType {
  constructor(arrowIdentifierData) {
    super();
    this.arrowIdentifierData = arrowIdentifierData;
  }
  eq(other) {
