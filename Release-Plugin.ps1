...
foreach ($p in @('package.json', 'manifest.json', 'versions.json', 'version-bump.mjs', 'styles.css')) {
    if (-not (Test-Path $p)) { throw "Fichier requis absent: $p" }
}
if (-not (Test-Path '.github/workflows/obsidian-plugin-release.yml')) {
    throw 'Workflow obsidian-plugin-release.yml absent.'
}
...
if (-not (Test-Path main.js)) { throw 'main.js absent après build.' }
